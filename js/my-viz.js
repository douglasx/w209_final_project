...
d3.select("label")
  ...
  var url = "data/iphone_screentime_data.json";
...
d3.json(url, function(error, data) {
  ...
  data.forEach(function(d) {
    ...
    d.hours = color.domain().map(function(name) {
      //return { mydate:mydate, name: name, y0: y0, y1: y0 += +d[name]}; });
      return {
        mydate:mydate,
        name: name,
        y0: y0,
   ...
   state.selectAll("rect")
    ...
    .enter().append("rect")
      ...
      .attr("y", function(d) {
        ...
        if (d.name === "") height_diff = 0; //reset for next d.mydate
  ...
  function restorePlot(d) {
    ...
    d3.selectAll(".class" + class_keep)
      .attr("x", function(d) { return x(d.mydate); })
      .transition()
      .duration(1000)
      .delay(function () {
  ...
  function change() {
    ...
    var x0 = x.domain(data.sort(sortDescending
        ...
        .map(function(d,i) { return d.date; }))
        .copy();

    state.selectAll(".class" + active_link)
         .sort(function(a, b) {
            return x0(a.mydate) - x0(b.mydate);
          });
